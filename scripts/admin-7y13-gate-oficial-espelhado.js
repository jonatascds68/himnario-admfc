/* ADMFC_7Y13_OFFICIAL_MIRROR_TEST */
/*
 * ADMFC_7Y12_OFFICIAL_MASTER_CLONE_GATE
 *
 * Simulação integral do futuro gate da Base Mestre.
 * O único alvo autorizado nesta etapa é um clone
 * byte a byte da Base Mestre oficial.
 */

/*
 * ADMFC_7Y11_OFFICIAL_MASTER_ABSOLUTE_BLOCK
 *
 * A Base Mestre oficial possui rota explícita.
 * Nesta etapa qualquer tentativa de APPLY contra
 * assets/base_mestre.json é bloqueada antes
 * de qualquer primitiva de escrita.
 */
  ...
  if (!process.exitCode) {
    ...
    if (
      ...
      if (baseFile === "assets/base_mestre.json") {
        console.error(
          'BLOQUEADO: Base Mestre real permanece protegida no 7Y.12.'
        );

        process.exitCode = 12;

      } else if (baseFile === "assets/base_mestre.SANDBOX-OFICIAL-7Y13.json") {
        /*
         * ADMFC 7Y.12:
         * gate completo exercitado exclusivamente
         * contra clone exato da Base Mestre.
         */
        if (!authorizeOfficial) {
          ...
          console.log(
            'AUTORIZAÇÃO DO CLONE OFICIAL: RECONHECIDA'
          );
          ...
          if (
            ...
            console.log(
              'COMMIT ATÔMICO DO CLONE: EXECUTADO'
            );
      ...
      } else if (baseFile === "assets/base_mestre.OFICIAL-SIMULADA-7Y43.json") {
        /*
         * PROTÓTIPO 7Y.2:
         *
    ...
    if (
      (
        ...
        baseFile === "assets/base_mestre.SANDBOX-OFICIAL-7Y13.json"
      )
    ) {
      console.log(
        'CHECKPOINT OFICIAL:',
