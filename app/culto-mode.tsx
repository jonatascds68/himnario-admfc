...
hymnAlignStorage.get().then(setHymnAlign);
  ...
  const bg = dark.surface, fg = dark.onSurface, gold = dark.brand;
  ...
  return (
    <View style={[styles.container, { backgroundColor: bg }]} testID="culto-mode-screen">
      <View style={styles.topBar}>
        ...
        <Text style={{ color: dark.muted, fontSize: 13, marginLeft: 6 }}>{queue.length > 0 ? `${pos + 1} / ${queue.length}` : ''}</Text>
        <View style={{ flex: 1 }} />
        <Pressable onPress={() => bumpFont(-0.1)} style={[styles.ctrl, { borderColor: dark.border }]} testID="culto-font-minus"><Text style={{ color: fg, fontWeight: '700', fontSize: 16 }}>A−</Text></Pressable>
        <Pressable onPress={() => bumpFont(0.1)} style={[styles.ctrl, { borderColor: dark.border, marginLeft: 8 }]} testID="culto-font-plus"><Text style={{ color: fg, fontWeight: '700', fontSize: 16 }}>A+</Text></Pressable>
      </View>
      {loading ? <ActivityIndicator color={gold} style={{ marginTop: SPACING.xxxl }} /> : hymn ? (
        <ScrollView contentContainerStyle={styles.scroll}>
          <Text style={{ color: dark.muted, fontSize: 12, fontWeight: '800', letterSpacing: 2 }}>{hymn.himnario.toUpperCase()}</Text>
